...
export default function App() {
  ...
  // Base address for BIN -> HEX conversion (default 0x00)
      ...
      try {
        ...
        for (const line of lines) {
          ...
          const calculatedChecksum =
            (0x100 -
              (bytes.slice(0, -1).reduce((acc, byte) => acc + byte, 0) &
                0xff)) &
            0xff;
        ...
        if (Object.keys(binaryData).length === 0) {
          throw new Error(
            'The HEX file does not contain any data records (type 00).'
          );
        }
  ...
  const convertBinToHex = (file) => {
    const startAddress = parseInt(baseAddress, 16);
  ...
  const createHexRecord = (address, recordType, dataBytes) => {
    ...
    return `:${toHex(byteCount)}${toHex(addressHi)}${toHex(addressLo)}${toHex(
      recordType
    )}${dataString}${toHex(checksum)}\n`;
  };
